        ...
        for loc in range(0, len(locs)):
            ...
            if (locs[loc] in loc_header) and (locs[loc] != -1):
                ...
                for k_iter in range(0, len(kernels_in_files[a - 1])):
                    ...
                    line = '\nhpx::future<void> op_par_loop_' + \
                        kernels[k]['name'] + '(char const *, hpx::future<op_set>,\n'
                    for n in range(1, kernels[k]['nargs']):
                        line = line + '  hpx::future<op_arg>,\n'
                    line = line + '  hpx::future<op_arg> );\n'
                    fid.write(line)
            ...
            if locs[loc] in loc_loops:
                ...
                for arguments in range(0, loop_args[curr_loop]['nargs']):
                    ...
                    if elem['type'] == 'op_arg_dat':
                        line = line + 'hpx::make_ready_future(' + elem['type'] + '(' + elem['dat'] + \
                            ',' + elem['idx'] + ',' + elem['map'] + \
                            ',' + elem['dim'] + ',' + elem['typ'] + \
                            ',' + elem['acc'] + ')),\n' + indent
                    elif elem['type'] == 'op_arg_gbl':
                        line = line + 'hpx::make_ready_future(' + elem['type'] + '(' + elem['data'] + \
                            ',' + elem['dim'] + ',' + elem['typ'] + \
                            ',' + elem['acc'] + ')),\n' + indent
