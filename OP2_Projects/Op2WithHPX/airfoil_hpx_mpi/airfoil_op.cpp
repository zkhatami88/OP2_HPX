...
//  fut1 = fn1(); fut2 = fut1.then([](...){fnc2();}); fut3 = fut2.then([](...){fnc3();})
   for(int k=0; k<2; k++) {

/*
	typedef std::vector<hpx::future<void>> future_vector;
	future_vector new_data1; 
	future_vector new_data2;
	future_vector new_data3;

	std::vector<hpx::future<void>> f;
	//hpx::future<void> f2=f.then([](hpx::future<void>) -> hpx::lcos::future<void> {});
	std::vector<hpx::future<void>> f2=f.then([](std::vector<hpx::future<void>>&) -> std::vector<hpx::future<void>> {});

	new_data1.push_back(op_par_loop_adt_calc("adt_calc",cells,
	...
	new_data2=new_data1.then([=](std::vector<hpx::future<void>>&&){

		//std::vector<hpx::future<void>> new_data2;

	     		new_data2.push_back(op_par_loop_res_calc("res_calc",edges,
...
//	});
	...
	/*hpx::when_all(new_data1B,
	[](const future_vector&){
		/*future_vector new_data1B;
